...
class PostureDialog(QMainWindow):
    ...
    def start(self):
        self.ui.pte_reports.appendHtml(
            '''<p><span style="color:white" style="font-size:16px"> Welcome to #Posture v1.2</span></p>''')
    ...
    def adding_nodes(self):
        ...
        if len(nodes) > 0:
            ...
            def debug():
                ...
                print("#%s | Node in Chain" % len(nodes_in_chain), nodes_in_chain)
                print("#%s | Independent nodes" % len(independent_nodes), independent_nodes)
                print("#%s | Out of Group" % len(group_heads), out_of_groups)
                print("#%s | Group members" % len(group_members), group_members)
                print("#%s | Group heads" % len(group_heads), group_heads)
                print("#%s | Head roots" % len(head_root), head_root)
                print("#%s | Result" % len(result_list), result_list)
                print("#%s | Result roots" % len(result_list), results_roots)
                print("#%s | Result in order" % len(self.ordered_selection_list), self.ordered_selection_list)
            ...
            if self.ui.lw_profiles.count() != 0:
                ...
                if profile_name in self.global_data:
                    ...
                    if replay == QMessageBox.Yes:
                        ...
                        self.ui.pte_reports.appendHtml(
                            '''<p><span style="color:orange"> <strong>"%s" </strong> deleted from collection.</span></p>''' % profile_name)
    ...
    def rename(self):
        ...
        if len(
            ...
            self.ui.pte_reports.appendHtml(
                '''<p><span style="color:#77e73a"> <strong>"%s" </strong> changed to:''' % old_name)
            self.ui.pte_reports.appendHtml(
                '''<p><span style="color:#77e73a"><strong>"%s"</strong></span></p>''' % new_name)
            ...
            if self.ui.chb_global.isChecked() and self.ui.chb_local.isChecked():
                ...
                if len(trubled_nodes) > 0:
                    self.ui.pte_reports.appendHtml(
                        '''<p><span style="color:red"> <strong>Error:</strong> </span></p>''')
                    self.ui.pte_reports.appendHtml(
                        '''<p><span style="color:orange"> Objects in the following list don't have parent: <br />Count: %s <br />Names: %s.</span></p>''' % len(
                            trubled_nodes), trubled_nodes)
                ...
                if self.ui.rb_global.isChecked() and self.ui.chb_realtime.isChecked():
                    ...
                    for i in range(len(self.global_data["%s" % selected]["ID"])):
                        ...
                        if mxs.isValidNode(node) != True:
                            ...
                            new_node.transform = mxs.execute(self.global_data["%s" % selected]["global_transform"][i])
                            new_node.wirecolor = mxs.execute(self.global_data["%s" % selected]["color"][i])

                            # new_node.name = mxs.execute(self.global_data["%s" % selected]["name"][i])
                            ...
                            # node.name = mxs.execute(self.global_data["%s" % selected]["name"][i])
                    ...
                    self.ui.pte_reports.appendHtml('''<p><span style="color:#77e73a">Global profile: <strong>"%s"</strong> applied to scene in:''' % selected)
                    self.ui.pte_reports.appendHtml('''<p><span style="color:#77e73a"><strong>"%s"</strong> Seconds.''' % round((time.time() - start_time),2))
                    ...
                    self.ui.pte_reports.appendHtml('''<p><span style="color:#77e73a">Local profile: <strong>"%s"</strong> applied to scene in:''' % selected)
                    self.ui.pte_reports.appendHtml('''<p><span style="color:#77e73a"><strong>"%s"</strong> Seconds.''' % round((time.time() - start_time),2))
                    ...
                    if len(recreated_list) > 0:
                        self.ui.pte_reports.appendHtml(
                            '''<p><span style="color:#999999">"Object in the following list are recreated: <br>"%s"</span></p>''' % recreated_list)
    ...
    def init_UI(self):
        ui_file = QFile(os.path.dirname(os.path.realpath(__file__)) + "\\Interface\\user-interface.ui")
